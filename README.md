...
| `rust-toolchain`     | Rust toolchain name to use, e.g. `stable`, `nightly` or `1.68.0`. It will be installed if necessary and used regardless of local overrides and the `rust-toolchain.toml` file. If the input is set to be an empty string, the action assumes some Rust toolchain is already installed and uses the default one. | `stable` |
